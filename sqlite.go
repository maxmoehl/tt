...
func (db *sqlite) getMultiple(table string, filter DatabaseFilter, orderBy OrderBy, target interface{}) error {
	...
	for rows.Next() {
		var item string
		if rows.Err() != nil {
			return fmt.Errorf("%w: %s", ErrInternalError, rows.Err().Error())
		}
		err = rows.Scan(&item)
		if err != nil {
			return fmt.Errorf("%w: %s", ErrInternalError, err.Error())
...
func (db *sqlite) update(table string, id string, value interface{}) error {
	...
	_, err = db.db.Exec(updateStmt, string(b), id)
	if err != nil {
		return fmt.Errorf("%w: %s", ErrInternalError, err.Error())
	}
	return nil
...
func (db *sqlite) SaveVacationDay(vacationDay VacationDay) error {
	vacationDay.ID = uuid.Must(uuid.NewRandom()).String()
...
func NewSQLite(dbFile string) (DB, error) {
	...
	if err != nil {
		return &sqlite{}, fmt.Errorf("%w: %s", ErrInternalError, err.Error())
	}
	err = db.Ping()
	if err != nil {
		return &sqlite{}, fmt.Errorf("%w: %s", ErrInternalError, err.Error())
